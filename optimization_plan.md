...
| 2026-05-16 | Reranker FT 设计 | `reranker_finetune_plan.md` 落地：986 train claims × (1 pos + 7 hard-negs from fused-no-rerank top-50), LoRA r=8 + classifier modules_to_save, InfoNCE list-wise loss, Gate A/B/C 三级判定。`scripts/build_reranker_ft_data.py` + `scripts/finetune_reranker.py` 上线。 | `reranker_finetune_plan.md` |
| 2026-05-16 | FT v1 (fp16) — 灾难性 NaN | seed 42, lr 2e-5, smoothing 0.05, n_negs 7, 3 ep. Step 20 起 loss=nan，AdamW 状态毒化。Eval recall 三个 ckpt 字节级相同（NaN 分数让 sorted 保留 input 顺序）。根因：纯 fp16 forward 在 XLM-R + InfoNCE 上溢出。修：fp16 → bf16 (4080 SUPER Ampere+ 原生支持，fp32 等价 dynamic range)。 | `outputs/reranker_train_seed42.log` |
| 2026-05-16 | FT v1 (bf16) — class-collapse 同款 | bf16 后训练稳定。Best r@20 = **0.338** @ step 200，base zero-shot ablation (r@20=0.333)。step 200 后回吐到 step 600 r@20=0.258。loss 从 4 (anti-corr) → 2 (= ln(8)，uniform random)。Gate A 0.40 未达；HM 期望增益 < +0.02。 | `outputs/reranker_train_seed42_bf16.log` |
| 2026-05-16 | FT v2 (低 LR + 高 smoothing) | seed 42, lr 5e-6, smoothing 0.15, n_negs 3, eval-top-k 100. Best r@20 = **0.326** @ step 500，比 v1 (0.338) 反而更差。低 LR + cosine schedule 让模型在 LR 衰到 ~0 才停，还没饱和。Gate A 未达。 | `outputs/reranker_train_seed42_v2.log` |
| 2026-05-16 | FT v3 (8 ep) — 灾难性遗忘 | seed 42, lr 1e-5, smoothing 0.10, n_negs 5, 8 ep, eval-top-k 100, eval-steps 100. Best r@20 = **0.325** @ step 200, step 200 后单调崩盘 → step 700 r@20=0.157 (低于 base zero-shot 一倍)。train loss 一路下到 1.4，eval recall 越训越烂 = 拟合 noise + 吃掉 pre-trained 通用语义。**3 次 FT 全在 0.32-0.34 撞墙**，证实容量 ceiling。 | `outputs/reranker_train_seed42_v3.log` |
| 2026-05-16 | v2-m3 zero-shot — 容量假设证伪 | `bge-reranker-v2-m3` (568M, 比 base 大 2×) zero-shot on diag_test, full pipeline. r@5 = 0.146 (略好于 base 0.119), r@10 = 0.195 (差于 base 0.210), **r@20 = 0.306** (差于 base 0.333, 比 no-rerank baseline 0.360 差 0.054). v2-m3 把 1-2 gold 顶进 top-5 代价是把其他 gold 推到 rank 21-50. 任何 MS-MARCO 系 reranker zero-shot 都负贡献，根因是 retrieval-relevance vs fact-checking entailment 任务 mismatch（不是模型容量）。详见 debug_log 复用经验 39。 | `outputs/eval_phase1/retrieval_ceiling_diag_test_ftrer.md` |
| 2026-05-16 | Reranker 路线放弃 / 转 chunking | 3 次 FT + v2-m3 zero-shot 全失败，确认 task mismatch 是结构性问题（不是超参 / 模型容量）。Qwen3-Reranker 同根（retrieval pre-train）不试。剩余 6 天转 chunking 路线（`scripts/profile_evidence.py` 已就绪，先看 verdict）。Reranker 全留 ablation narrative。 | `reranker_finetune_plan.md` §15 |
| 2026-05-16 | Chunking verdict — Unlikely to help | `scripts.profile_evidence` over 1,208,827 passages：median 26 tokens, p99 80 tokens, **% over 256 tokens = 0.00% (仅 43/1.2M)**。corpus 已被原作者切到单句原子粒度（Wikipedia/新闻片段，p90 sentences-per-passage=1）。bge-m3 encoder (256 max) 没有截断；reranker (512) 没有截断。chunking 没有可切割的空间 → 不做。**这是 valuable negative result**：retrieval 瓶颈不是结构性的 truncation，而是语义/词面匹配（HyDE 已试，reranker 已试）。详见 `chunking_plan.md` §13。 | `outputs/evidence_profile.md` |
| 2026-05-16 | Retrieval-side 路线全部走完 / 转 Phase 6 | 4 levers 全失败：HyDE/sub-claim（只对 @50/@100，#36）+ Reranker FT/zero-shot（task mismatch, #39）+ Chunking（corpus 已原子化）。**Lock Track 2 v1 (fused no-rerank, final_k=20, prompt v1, HM 0.213) 为最终生产配置**。剩余 6 天专注：(a) official_dev 终评（D-006 配额最后 1 次）；(b) test set 预测；(c) 报告写作。所有 ablation 留 narrative。 | `optimization_plan.md` §10 |
